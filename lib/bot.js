...
function generateMsg(configUrl, issue, prefix, summary) {
  return prefix + '<' + configUrl + issue.key + '|' + issue.key + '>' + ' - ' + summary;
}
              ...
              (function (def, jira) {
                ...
                jiraApi.findIssue(jira, function (error, issue) {
                  ...
                  if (issue && self.config.post == true) {
                    var configUrl = getBaseUrl(prjName, self.config);
                    msg.push(generateMsg(configUrl, issue, self.config.custom_texts.messagePrefix, (issue.fields.summary || '')));
                    if (self.config.showIssueDetails && expandInfo) {
                      if (verbose) {
                        console.log(issue);
                      }
                      if (self.config.issueDetailsToShow['fields.summary'] && issue.fields.summary) {
